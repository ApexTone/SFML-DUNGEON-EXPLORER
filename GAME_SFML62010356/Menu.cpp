...
HowToPlay::HowToPlay(float width, float height) {
  ...
  bg.setScale(Vector2f(RWWIDTH / bg.getGlobalBounds().width, RWHEIGHT /
  bg.getGlobalBounds().height));
  */
}
...
Pause::Pause(RenderWindow &rw) {
  ...
  frame.setPosition((float)(rw.getSize().x / 2) - 20,
                    (float)(rw.getSize().y / 2) - 20);
  frame.setOrigin(xSize / 2, ySize / 2);
  if (!font.loadFromFile("Serithai-Regular.ttf")) {
    // Handle error
  }
  asking.setFont(font);
  asking.setPosition((float)(rw.getSize().x / 2) - 310,
                     (float)(rw.getSize().y / 2) - 40);
