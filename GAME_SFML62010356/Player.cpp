...
Player::Player(Texture* texture, Vector2u imageCount, float switchTime, float speed) :
	animation(texture, imageCount, switchTime)//Also use the variables passed with player class constructor
{
	Texture tex;
	...
	body.setOrigin(Vector2f(xSize/2,ySize/2));//set origin to compatible with AABB collision check
	
	body.setTexture(texture);//set texture of player (no & since texture here is pointer)
	...
	hitbox.setSize(Vector2f(xSize-35,ySize-7));//Only work for 100,100 body size!
	...
	punchHitbox.setOrigin(punchHitbox.getSize().x / 2, punchHitbox.getSize().y / 2);
...
Collider Player::GetPunchCollider()
{
	return Collider(punchHitbox);
}
...
void Player::Update(float deltaTime,RenderWindow& rw)
{
	...
	Vector2f movement(0.0f, 0.0f);//Every time this method is called, movement will always be (0,0)
	...
	else if (Mouse::isButtonPressed(Mouse::Right))
	{
		if (fireRateControl>=10)//Will fire at 4 shots/sec (40/10)
		{
			...
			mousePos = Vector2f(rw.mapPixelToCoords(Mouse::getPosition(rw)).x, rw.mapPixelToCoords(Mouse::getPosition(rw)).y);
	...
	{
		//cout << i << "#\t" << bullets[i].shape.getPosition().x << " " << bullets[i].shape.getPosition().y << endl;
		bullets[i].shape.move(bullets[i].currVelocity);
		if (bullets[i].shape.getPosition().x < 0 || bullets[i].shape.getPosition().y < 0 || bullets[i].shape.getPosition().x >= rw.getSize().x || bullets[i].shape.getPosition().y >= rw.getSize().y)
		{
	...
	if (movement.x == 0.0f && movement.y==0.0f && !action)//When idle
	{
		...
		Vector2f mousePos = Vector2f(rw.mapPixelToCoords(Mouse::getPosition(rw)).x,rw.mapPixelToCoords(Mouse::getPosition(rw)).y);
...
void Player::setBulletTexture(Texture* tex)
{
	b.setBulletTexture(tex);
}
