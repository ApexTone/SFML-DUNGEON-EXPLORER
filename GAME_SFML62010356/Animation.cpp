...
bool Animation::playRow(int row, float deltaTime,bool faceRight)
{
	...
	if (totalTime >= toSwitch)//Change the image
	{
		...
		uvRect.top = currentRowImage.y * uvRect.height;//shift image area that is being use
		if (faceRight)
		{
		...
		{
			uvRect.left = (currentRowImage.x + 1) * abs(uvRect.width);//Start from right side of the image to make it reverse
			uvRect.width = -abs(uvRect.width);//Reverse the render image
		}
	}
	return true;
