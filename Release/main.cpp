...
bool sortinrev(const pair<int, string> &a,
               const pair<int, string> &b); // descending vector sort
...
int main() {
  ...
  */
  ...
  bool win = false;
  RenderWindow rw(VideoMode(RWWIDTH, RWHEIGHT), "Dungeon Explorer 62010356",
  ...
  bg.setTextureRect(
      IntRect(0, 0, backgroundTex.getSize().x, backgroundTex.getSize().y));
  bg.setScale(Vector2f(RWWIDTH / bg.getLocalBounds().width,
                       (RWHEIGHT - 60) / bg.getLocalBounds().height));
  ...
  wallVector.push_back(Platform(nullptr, Vector2f(RWWIDTH * 3, 100),
                                Vector2f(0, RWHEIGHT - 20)));
  wallVector.push_back(
      Platform(nullptr, Vector2f(RWWIDTH * 3, 40), Vector2f(0, 5)));
  wallVector.push_back(
      Platform(nullptr, Vector2f(40, RWHEIGHT * 3), Vector2f(RWWIDTH - 20, 0)));
  wallVector.push_back(
      Platform(nullptr, Vector2f(40, RWHEIGHT * 3), Vector2f(0, 5)));
  ...
  */
  ...
  // vampireVector.push_back(new
  // Vampire(&vampireTexture,Vector2u(4,4),0.1f,80.0f));
  ...
  winText.setOrigin(Vector2f(winText.getGlobalBounds().width / 2,
                             winText.getGlobalBounds().height / 2));
  ...
  loseText.setOrigin(Vector2f(loseText.getGlobalBounds().width / 2,
                              loseText.getGlobalBounds().height / 2));
  ...
  ask4name.setOrigin(Vector2f(ask4name.getGlobalBounds().width / 2,
                              ask4name.getGlobalBounds().height / 2));
  ...
  highscoreBackground.setTextureRect(
      IntRect(0, 0, hsBgText.getSize().x, hsBgText.getSize().y));
  highscoreBackground.setTexture(hsBgText);
  highscoreBackground.setScale(
      Vector2f(RWWIDTH / highscoreBackground.getGlobalBounds().width,
               RWHEIGHT / highscoreBackground.getGlobalBounds().height));
  ...
  highscoreTitle.setOrigin(highscoreTitle.getGlobalBounds().width,
                           highscoreTitle.getGlobalBounds().height);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    highscoreNameText[i].setPosition(
        Vector2f((float)RWWIDTH / 4, (float)RWHEIGHT / 6 * (i + 1)));
    ...
    highscoreScoreText[i].setPosition(
        Vector2f((float)3 * RWWIDTH / 4, (float)RWHEIGHT / 6 * (i + 1)));
  }
  ...
  do {
    ...
    pairScoreName.push_back(
        make_pair(atoi(highscoreScore.c_str()), highscoreName));
  ...
  htpBG.setScale(Vector2f(1.05 * RWWIDTH / htpBG.getGlobalBounds().width,
                          1.1 * RWHEIGHT / htpBG.getGlobalBounds().height));
  ...
  levelBackFrame.setOrigin(backFrame.getSize().x / 2,
                           backFrame.getSize().y / 2);
  ...
  pressQ.setOrigin(pressQ.getGlobalBounds().width / 2,
                   pressQ.getGlobalBounds().height / 2);
  // Game Loop
  while (rw.isOpen()) {
    ...
    if (e.type == Event::KeyReleased)
    {
            ...
            {
                    tankVector.push_back(new Tank(&tankTexture, Vector2u(4, 4),
    0.1f, 60.0f));
            }
            if (e.key.code == Keyboard::U)
            {
                    vampireVector.push_back(new Vampire(&vampireTexture,
    Vector2u(4, 4), 0.08f, 90.0f));
            }
            if (e.key.code == Keyboard::I)
            {
                    casterVector.push_back(new Caster(&casterTexture,
    Vector2u(4, 4), 0.1f, 80.0f));
            }
    }
    */
    ...
    else if (gameState == 1000) // Waiting First level
    {
      ...
      levelName.setOrigin(levelName.getGlobalBounds().width / 2,
                          levelName.getGlobalBounds().height / 2);
      rw.draw(levelName);
      rw.draw(pressQ);
      if (e.type == Event::KeyReleased) {
    ...
    } else if (gameState == 1) // First Level
    {
      ...
      if (totalTime >= spawnTime) {
        ...
        if (randomNumber == 1) {
          if (slvl[0].tank++ < lvl[0].tankNO)
            tankVector.push_back(
                new Tank(&tankTexture, Vector2u(4, 4), 0.1f, 60.0f));
          else if (slvl[0].vampire++ < lvl[0].vampireNO)
            vampireVector.push_back(
                new Vampire(&vampireTexture, Vector2u(4, 4), 0.08f, 90.0f));
          else if (slvl[0].caster++ < lvl[0].casterNO)
            casterVector.push_back(
                new Caster(&casterTexture, Vector2u(4, 4), 0.1f, 80.0f));
        }
        if (randomNumber == 2) {
      ...
      for (size_t i = 0; i < tankVector.size(); i++) // Tank OOB
      {
        for (size_t j = 0; j < wallVector.size(); j++) {
          wallVector[j].GetCollider().CheckCollision(
              tankVector[i]->GetCollider(), 1.0f);
        }
      }
      for (size_t i = 0; i < vampireVector.size(); i++) // Vampire OOB
      {
        for (size_t j = 0; j < wallVector.size(); j++) {
          wallVector[j].GetCollider().CheckCollision(
              vampireVector[i]->GetCollider(), 1.0f);
        }
      }
      for (size_t i = 0; i < casterVector.size(); i++) // Caster OOB
      {
        for (size_t j = 0; j < wallVector.size(); j++) {
          wallVector[j].GetCollider().CheckCollision(
              casterVector[i]->GetCollider(), 1.0f);
        }
      }
      ...
      for (size_t i = 0; i < p.bulletVectorSize();
           i++) // Bullet-Vampire collision
      {
      ...
      // TAKING DAMAGE
      for (size_t i = 0; i < tankVector.size();
           i++) // Tank-Player Punch Intersect
      {
      ...
      for (size_t i = 0; i < vampireVector.size();
           i++) // Vampire-Player Punch Intersect
      {
      ...
      // PUNCH INTERSECTION
      for (size_t i = 0; i < tankVector.size();
           i++) // Player-Tank Punch Intersect
      {
        if (p.GetPunchCollider().CheckIntersect(tankVector[i]->GetCollider()) &&
            p.punching()) {
      ...
      for (size_t i = 0; i < vampireVector.size();
           i++) // Player-Vampire Punch Intersect
      {
        if (p.GetPunchCollider().CheckIntersect(
                vampireVector[i]->GetCollider()) &&
            p.punching()) {
          // cout << "Punch Vampire!" << endl;
          vampireVector[i]->takeDamage(p.meleeDamage());
        }
      }
      for (size_t i = 0; i < casterVector.size();
           i++) // Player-Caster Punch Intersect
      {
        if (p.GetPunchCollider().CheckIntersect(
                casterVector[i]->GetCollider()) &&
            p.punching()) {
      ...
      for (size_t i = 0; i < p.bulletVectorSize(); i++) {
        if (p.getBulletPosition(i).x > rw.getSize().x ||
            p.getBulletPosition(i).y > rw.getSize().y ||
            p.getBulletPosition(i).x < 0 || p.getBulletPosition(i).y < 0) {
          // cout << p.getBulletPosition(i).x << "   " <<
          // p.getBulletPosition(i).y << endl;
      ...
      for (size_t i = 0; i < casterVector.size(); i++) {
        for (size_t b = 0; b < casterVector[i]->bulletVectorSize(); b++) {
          if (casterVector[i]->getBulletPosition(b).x > rw.getSize().x ||
              casterVector[i]->getBulletPosition(b).y > rw.getSize().y ||
              casterVector[i]->getBulletPosition(b).x < 0 ||
              casterVector[i]->getBulletPosition(b).y < 0) {
    ...
    else if (gameState == 1001) // Waiting second level
    {
      ...
      if (e.type == Event::KeyReleased) {
        if (e.key.code == Keyboard::Q) {
          ...
          case 1:
            p.damageBuff();
            enemySpawnDivider = 1.5f;
            break; // spawn rate to 1 sec/spawn
          case 2:
            p.healthBuff();
            enemySpeedMult = 1.2f;
            break;
      ...
      if (totalTime >= spawnTime / enemySpawnDivider) {
        ...
        if (randomNumber == 1) {
          if (slvl[1].tank++ < lvl[1].tankNO)
            tankVector.push_back(new Tank(&tankTexture, Vector2u(4, 4), 0.1f,
                                          60.0f * enemySpeedMult));
          else if (slvl[1].vampire++ < lvl[1].vampireNO)
            vampireVector.push_back(new Vampire(&vampireTexture, Vector2u(4, 4),
                                                0.08f, 90.0f * enemySpeedMult));
          else if (slvl[1].caster++ < lvl[1].casterNO)
            casterVector.push_back(new Caster(&casterTexture, Vector2u(4, 4),
                                              0.1f, 80.0f * enemySpeedMult));
        }
        if (randomNumber == 2) {
    ...
    } else if (gameState == 99) // End game state: Name+Score (NAME CAN'T EXCEED
                                // 10 LETTERS (field limit size))
    {
      for (int i = 0; i < 3; i++) {
        slvl[i].caster = 0;
        slvl[i].tank = 0;
      ...
      if (blinkState) {
        bar.setPosition(playerNameText.getGlobalBounds().width + 5 - 140 +
                            RWWIDTH / 2,
                        RWHEIGHT / 2);
        rw.draw(bar);
      }
      ...
      {
        if (lastChar != e.text.unicode && playerName.length() < 10 &&
        ...
        } else if (lastChar != e.text.unicode && e.text.unicode == 8 &&
                   playerName.length() > 0) // Delete letter
        {
          lastChar = e.text.unicode;
      ...
      if (e.type == Event::KeyReleased) {
        if (e.key.code == Keyboard::Return) {
          ...
          {
                  //cout << pairScoreName[i].first << "," <<
          pairScoreName[i].second << "\n";
          }
          */
          ...
          for (size_t i = 0; i < 5; i++) // only write top 5 score
          {
            highscoreWriter << pairScoreName[i].first << ","
                            << pairScoreName[i].second << "\n";
          }
...
bool sortinrev(const pair<int, string> &a,
               const pair<int, string>
                   &b) // Sort in descending order consider first element
{
